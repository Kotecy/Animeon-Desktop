...
const CHANGELOG = [
  { version: '0.4.10', date: '22.09.2026', items: [
    ...
    'Журнал работы стал вместительнее, показывает до 30 записей и сохраняет размер блока при обновлении',
    'Добавлена функция очистки кэша без сброса авторизации и сохранённых профилей',
    'Обновлен локальный список секреток',
    'Улучшена адресная строка и выравнивание элементов интерфейса'
  ] },
...
export default function Settings({ baseUrl, onBaseUrl }) {
  ...
  const handleClearCache = async () => {
    ...
    try {
      ...
      setMsg('Не удалось очистить кэш')
    } finally {
      setCacheBusy(false)
    }
  }
  ...
  useEffect(() => {
    const el = changelogDialog.current
    if (!el) return
    if (showChangelog && !el.open) el.showModal?.()
    else if (!showChangelog && el.open) el.close?.()
  }, [showChangelog])
  ...
  return <div className="signal-page">
    ...
    <div className="signal-settings">
      <div className="signal-column">
        <section className="signal-card">
          <div className="signal-heading signal-nya-anchor">
            <h2>Профили</h2>
            {nyaVisible && <span className="signal-nya-code" role="status">NYA-D7E6-0187</span>}
            <small>{accounts.length} / 4</small>
          </div>
          <p className="signal-note">Отдельная сессия для каждого аккаунта.</p>
          <div className="signal-profiles-v2">
            {accounts.map(profile => {
              const active = Number(profile.id) === acc
              const name = profile.nickname || 'Профиль ' + profile.id
              return (
                <div key={profile.id} className={'signal-profile-item' + (active ? ' active' : '')}>
                  <button
                    type="button"
                    className="signal-profile-btn"
                    aria-pressed={active}
                    onClick={() => saveAccount(profile.id)}
                  >
                    <span className="signal-face">{name[0].toUpperCase()}</span>
                    <div className="signal-profile-info">
                      <b>{name}</b>
                      <span className={profile.nickname ? 'signed-in' : 'signal-note'}>
                        {profile.nickname ? '● Вход выполнен' : '○ Не авторизован'}
                      </span>
                    </div>
                    <span className="signal-profile-badge">{active ? 'выбрано' : 'выбрать'}</span>
                  </button>
                  <details className="signal-profile-menu" open={openProfileMenu === String(profile.id)}>
                    <summary
                      aria-label={'Действия профиля ' + name}
                      aria-expanded={openProfileMenu === String(profile.id)}
                      onClick={event => {
                        event.preventDefault()
                        setOpenProfileMenu(current => current === String(profile.id) ? null : String(profile.id))
                      }}
                    ><MoreIcon /></summary>
                    <button disabled={profileBusy} onClick={() => removeAccount(profile.id)}>
                      {Number(profile.id) === 1 ? 'Очистить профиль' : 'Удалить профиль'}
                    </button>
                  </details>
                </div>
              )
            ...
            {accounts.length < 4 && (
              <button
                ...
                onClick={addAccount}
                className="signal-profile-add-v2"
              >
                <span>+</span>
                <span>{profileBusy ? 'Добавление…' : 'Добавить профиль'}</span>
              </button>
            )}
          </div>
        </section>

        <section className="signal-card signal-cache-card">
          <div className="signal-heading">
            <h2>Очистка кэша</h2>
          </div>
          <p className="signal-note">Очищает кэшированные файлы и изображения страниц. Сессии и профили сохраняются.</p>
          <button
            type="button"
            className={'signal-cache-btn' + (cacheDone ? ' done' : '')}
            disabled={cacheBusy}
            onClick={handleClearCache}
          >
            {cacheBusy ? 'Очистка кэша…' : cacheDone ? 'Кэш очищен' : 'Очистить кэш'}
          </button>
        </section>
      </div>
      ...
      <div className="signal-column">
        <section className="signal-card">
          ...
          <p className="signal-note">Для новых вкладок AnimeOn.</p>
          <div className="signal-envs">
            {['https://v1.animeon.co', 'https://v2.animeon.co'].map(value => (
              <button
                key={value}
                type="button"
                aria-pressed={url === value}
                onClick={() => selectBase(value)}
              >
                <b>{formatSiteBaseUrl(value)}</b>
                <small>{url === value ? 'выбрано' : 'выбрать'}</small>
              </button>
            ))}
          </div>
        </section>

        <section className="signal-card signal-about">
          <div className="signal-heading">
            <h2>О приложении</h2>
          </div>
          <div className="signal-brand">
            ...
            <div>
              <b>AnimeOn Desktop</b>
              <p className="signal-note">v{version}</p>
            </div>
          </div>
          <div className="signal-update-wrap">
            <button
              type="button"
              disabled={checking}
              onClick={async () => {
                if (updateUrl) { window.api?.appOpenUrl?.(updateUrl); return }
                setChecking(true); setMsg('Проверяем...'); setUpdateUrl('')
                try {
                  const result = await window.api?.appCheckUpdate?.()
                  if (!result?.ok) setMsg(result?.error || 'Не удалось проверить обновления')
                  else if (result.newer) {
                    setMsg(`Доступна новая версия v${result.latest}`)
                    setUpdateUrl(result.url || 'https://github.com/Kotecy/Animeon-Desktop/releases')
                  } else setMsg(`Установлена актуальная v${result.current}`)
                } catch { setMsg('Не удалось проверить обновления') }
                setChecking(false)
              }}
              className="signal-primary signal-update-btn"
            >
              {checking ? 'Проверяем...' : updateUrl ? 'Установить новую версию ↗' : 'Проверить обновления'}
            </button>
            {msg && <p className="signal-note signal-update-msg" role="status">{msg}</p>}
          </div>
          <button
            type="button"
          ...
          <div className="signal-credits">
            <h3>Credits</h3>
            {[['Приложение', 'nieqq', true], ['Отдельная благодарность', 'Suchka322', false]].map(([role, nick, withBy]) => (
              <div key={nick}>
                <span className="signal-note">{role}</span>
                <a
                  href={normalizeSiteBaseUrl(baseUrl) + '/user/' + encodeURIComponent(nick)}
                  onClick={event => {
                    event.preventDefault()
                    window.api?.tabsCreate?.(normalizeSiteBaseUrl(baseUrl) + '/user/' + encodeURIComponent(nick))
                  }}
                >
                  {withBy ? 'by ' : ''}{nick} ↗
                </a>
    ...
    <dialog
      ref={changelogDialog}
      className="signal-changelog-dialog"
      aria-labelledby="changelog-title"
      onCancel={() => setShowChangelog(false)}
      onClose={() => setShowChangelog(false)}
      onClick={event => {
        const rect = changelogDialog.current?.getBoundingClientRect()
        if (rect && (event.clientX < rect.left || event.clientX > rect.right || event.clientY < rect.top || event.clientY > rect.bottom)) {
          setShowChangelog(false)
        }
      }}
    >
      <div className="signal-changelog-header">
        <div>
          <span className="signal-kicker">ОБНОВЛЕНИЯ</span>
          <h2 id="changelog-title">История изменений</h2>
        </div>
        <button
          type="button"
          className="signal-changelog-close"
      ...
      <div className="signal-changelog-list">
        {CHANGELOG_BY_DATE.map((group, index) => {
          const isOpen = expandedLog === group.date
          const isLatest = index === 0
          return (
            <div key={group.date} className={'signal-history-group' + (isOpen ? ' is-open' : '') + (isLatest ? ' is-latest' : '')}>
              <button
                type="button"
                className="signal-history-date-btn"
                aria-expanded={isOpen}
                onClick={() => setExpandedLog(current => current === group.date ? '' : group.date)}
              >
                <div className="signal-history-date-left">
                  <span className="signal-history-chevron" aria-hidden="true">
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                      <polyline points="9 18 15 12 9 6" />
                    </svg>
                  </span>
                  <span className="signal-history-date-text">{group.date}</span>
                  {isLatest && <span className="signal-history-tag-latest">Актуальное</span>}
                </div>
                <div className="signal-history-date-pills">
                  {group.releases.map(r => (
                    <span key={r.version} className="signal-history-version-pill">v{r.version}</span>
                  ))}
                </div>
              </button>
              {isOpen && (
                <div className="signal-history-content">
                  {group.releases.map(release => (
                    <div className="signal-history-release" key={release.version}>
                      {group.releases.length > 1 && <div className="signal-history-subversion">Версия v{release.version}</div>}
                      <ul className="signal-history-bullets">
                        {release.items.map(item => (
                          <li key={item}>
                            <span className="signal-bullet-dot" aria-hidden="true" />
                            <span>{item}</span>
                          </li>
                        ))}
                      </ul>
