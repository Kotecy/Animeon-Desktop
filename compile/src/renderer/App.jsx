...
export default function App() {
  ...
  const pushEvent = (text) => setEvents(prev => [{ id: Date.now() + Math.random(), text, time: moscowTime() }, ...prev].slice(0, 30))
  ...
  useEffect(() => {
    ...
    window.api?.utilitiesList?.().then(items => { if (Array.isArray(items)) setUtilities(items) }).catch(() => {})
    const unsubscribeUtilities = window.api?.onUtilitiesUpdated?.(items => setUtilities(Array.isArray(items) ? items : []))
    ...
    return () => {
      for (const unsubscribe of [unsubscribeTabsLimit, unsubscribeFollowbackDone, unsubscribeFollowbackTick, unsubscribeAnomaly, unsubscribeTabs, unsubscribeNavigationBlocked, unsubscribeUtilities]) {
  ...
  useEffect(() => {
    const key = event => {
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'k') {
        event.preventDefault(); setCommandsOpen(open => !open); setCommandQuery(''); setCommandError('')
      }
    ...
    return () => { window.removeEventListener('keydown', key); unsub?.() }
  }, [])
  ...
  const commands = [
    {
      name: 'Детектор',
      description: 'Поиск аномалий (вкл/выкл)',
      run: async () => {
        try {
          const result = await window.api?.detectorToggle()
          if (result?.error) {
            setCommandError(result.error)
            pushEvent(`Детектор: ${result.error}`)
            return
          }
          pushEvent(result?.watching ? 'Наблюдение включено' : 'Наблюдение выключено')
          setCommandsOpen(false)
        } catch {
          setCommandError('Не удалось переключить детектор')
          pushEvent('Детектор: не удалось изменить состояние')
        }
      }
    },
    ...utilities.map(item => ({
      name: item.name,
      description: item.description || 'Пользовательская функция',
      run: async () => {
        try {
          ...
          setCommandError('Не удалось переключить функцию')
  ...
  return (
    ...
    {commandsOpen && <div className="signal-overlay" onClick={() => setCommandsOpen(false)}><section role="dialog" aria-modal="true" aria-label="Найти функцию" className="signal-palette" onClick={event => event.stopPropagation()}><input autoFocus value={commandQuery} onChange={event => setCommandQuery(event.target.value)} placeholder="Найти функцию…" />{commands.map(command => <button className="signal-command-item" key={command.name} onClick={command.run}><span className="signal-command-glyph" aria-hidden="true"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 3a3 3 0 0 0-3 3v12a3 3 0 0 0 3 3 3 3 0 0 0 3-3 3 3 0 0 0-3-3H6a3 3 0 0 0-3 3 3 3 0 0 0 3 3 3 3 0 0 0 3-3V6a3 3 0 0 0-3-3 3 3 0 0 0-3 3 3 3 0 0 0 3 3h12a3 3 0 0 0 3-3 3 3 0 0 0-3-3z"/></svg></span><span><b>{command.name}</b><small>{command.description}</small></span></button>)}{!commands.length && <p>Ничего не найдено</p>}{commandError && <p role="alert">{commandError}</p>}</section></div>}
