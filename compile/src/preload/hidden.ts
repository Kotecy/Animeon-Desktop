...
try {
  contextBridge.exposeInMainWorld('__animeon', {
    ...
    onCollectorEvent: (cb: (data: any) => void) => {
      const listener = (_e: any, d: any) => cb(d)
      ipcRenderer.on('anomaly:collect-result', listener)
      return () => ipcRenderer.removeListener('anomaly:collect-result', listener)
    },
    onDetectorUpdated: (cb: (data: any) => void) => {
      const listener = (_e: any, d: any) => cb(d)
      ipcRenderer.on('detector:updated', listener)
      return () => ipcRenderer.removeListener('detector:updated', listener)
    },
    onOpenDetectorModal: (cb: () => void) => {
      const listener = () => cb()
      ipcRenderer.on('detector:open-modal', listener)
      return () => ipcRenderer.removeListener('detector:open-modal', listener)
    },
    triggerDetectorToggle: (enabled?: boolean) => ipcRenderer.invoke('detector:toggle', enabled),
    getTabsCount: () => ipcRenderer.invoke('tabs:getCount'),
  })
} catch {}
