...
async function main() {
  ...
  assert.doesNotMatch(dashboardSource, /XP Монитор/)
  assert.match(dashboardSource, /signal-custom-grid/)
  assert.equal((pages.dashboard.match(/signal-custom-grid/g) || []).length, 0, 'Tools start collapsed')
  ...
  assert.equal(JSON.parse(fs.readFileSync(path.join(root, 'package.json'))).version, '0.4.10')
  assert.equal(JSON.parse(fs.readFileSync(path.join(root, 'package-lock.json'))).version, '0.4.10')
